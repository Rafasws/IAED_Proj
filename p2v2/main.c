...
void flights_actions(){
    ...
    ptrFlight flight = (ptrFlight) malloc(sizeof(struct Flight));
    scanf("%c", &a);
    if(a == '\n'){
        print_all_flights();
    }
...
int add_flight(ptrFlight flight){
    ...
    int i = 0;
    if(strlen(flight->code) < 3|| strlen(flight->code) > SIZE_FLIGHT_CODE){
    ...
    flights[flights_counter] = flight;
    return 0;
}
...
void print_all_flights(){
    ...
    for(i = 0; i < flights_counter; i++){
        ...
        flights[i]->ap_departure, flights[i]->ap_arrival, res[2], res[3], 
        res[4], res[0], res[1]);
    }
}
...
void print_flights_dp_airport(){
    ...
    for (i = 1; i < j; i++) {
        k = arr[i];
        w = i - 1;
        while (w >= 0 && ((flights[arr[w]]->hour + flights[arr[w]]->date) > 
        (flights[k]->hour + flights[k]->date))){
            arr[w + 1] = arr[w];
            w --;
        }
        arr[w + 1] = k;
...
void print_flights_ar_airport(){
    ...
    for (i = 1; i < j; i++) {
        ...
        w = i - 1;
        while (w >= 0 && ((flights[arr[w]]->hour + flights[arr[w]]->date + 
        flights[arr[w]]->duration) > (flights[k]->hour + flights[k]->date + 
        flights[k]->duration))){
            arr[w + 1] = arr[w];
            w--;
        }
